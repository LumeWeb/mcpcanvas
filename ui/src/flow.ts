...
//
// This is the single source of truth for the flow that the host-rendered HTML
// shells drive. It is deliberately free of DOM and MCP-transport concerns so it
// can be unit-tested in node with a stubbed callTool.
//
// States:
...
export interface FlowConfig {
  ...
  // exposes how to cancel the in-progress handle and start fresh.
   ...
   * Only flow apps that reuse in-use logins supply it. */
  inUseMsg?: string;
}
 ...
 * handler), then from `structuredContent.error` (the host ErrorResult shape
...
export function createFlowMachine(cfg: FlowConfig, callTool: CallTool) {
  ...
  // UI renders alreadyDoneMsg rather than doneMsg.
  ...
  // Revoke the in-progress flow by its handle (single-flight flows). Only
  // present when the config supplies a revoke tool.
