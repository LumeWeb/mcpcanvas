...
//
// Most OOB apps are async: start -> poll status by handle -> done. Some apps
// instead run the whole change synchronously in the human's browser on a hosted
// page: after the tool mints the page there is nothing for the app to poll. The
// model calls the start tool, gets back a needs_human hand-off carrying an
// action_url, and the human opens that page, fills the form, and is done. This
// machine drives that "click -> mint -> show the deep link" lifecycle with no
// poll loop.
...
export function createLinkMachine(cfg: LinkConfig, callTool: CallTool) {
  ...
  // (e.g. an intermediate sign-in page) rather than minting a fresh page.
