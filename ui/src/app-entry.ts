// Shared app entrypoint helper: build a FlowConfig/LinkConfig-driven machine
// and wire it to elements. Each per-app entry supplies its own config/ids; the
// heavy flow logic lives in flow.ts / link.ts and is tested there, so these
// entries stay thin.
 ...
 * its element-id map. Flow apps and the one-shot link apps differ only in the
...
export interface AppDefinition<Config, Ids extends Record<string, string>> {
  name: string;
  config: Config;
  /** Element ids referenced by the host-rendered HTML shell. */
  ids: Ids;
}
...
// Produces the status class/message and the pending flag; a null statusState
// means "leave the status element alone" (idle). The url readout is refreshed
// independently in runAppEntry.
...
/** Element ids every flow app's host-rendered shell references. */
 ...
 * defaults), wire the flow machine to the host-rendered elements, and either
 * run synchronously with a caller-supplied `callTool` (tests/demo) or connect
 * to the host over postMessage via bootApp, advertising the runtime build
 * version.
 */
...
/** Element ids every link app's host-rendered shell references. */
 ...
 * for synchronous out-of-band changes with no poll loop.
 */
